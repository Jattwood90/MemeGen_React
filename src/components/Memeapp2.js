// This version can be pasted into Memeapp.js in order to run the Django based API. 
// I have noted where the changes were made in order to load a different API.
...
//     useEffect(() => {
//         try {
//             fetch('http://127.0.0.1:8000/%5Eapi/item/').then(x => //changes here
...
//                 {memes.map(response => {
//                     return (
//                     <img onClick={() => setMeme(((meme) => [response.image]))} //changes here
//                         className="mmbox" key={response.id} 
//                         src={response.image} alt={response.title} //changes here
//                         id="image-change" >
//                     </img>);         
//                     })}
